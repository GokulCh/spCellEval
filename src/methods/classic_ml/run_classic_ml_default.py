...
def run_on_datasets(main_dir, dataset_path, model, kfold_method, granularity_level, random_state, n_jobs_model, model_kwargs, verbose, scaling, dumb_columns, output_path):
    """
    ...
    if main_dir==None and dataset_path==None:
        raise Exception("No dataset was suppplied.")
    ...
    if main_dir is not None:
        ...
        kfold_dir = os.path.join(dataset_path, f'quantification/processed/kfolds_{kfold_method}_{granularity_level}')
        if not os.path.isdir(output_path):
            os.makedirs(output_path)

        label_dir = os.path.join(dataset_path, f'quantification/processed/labels_{kfold_method}_{granularity_level}.csv')
...
def main():
    ...
    parser.add_argument(
        "--dataset_path",
        type=str,
        help="Path to a specific dataset to analyze. Should point to the dataset directory containing processed/. If set, renders main_dir argument useless.",
        default=None
    )
    parser.add_argument(
        "--model",
